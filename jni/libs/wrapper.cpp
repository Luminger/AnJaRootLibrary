...
jlongArray jni_capget(JNIEnv* env, jobject obj, jint pid)
{
    ...
    try
    {
        util::Capabilities caps = util::getCapabilities(pid);
        ...
        jlong inheritable)
{
    ...
    try
    {
        util::Capabilities caps(effective, permitted, inheritable);
        util::setCapabilities(caps);
    }
    catch(std::system_error& e)
    {
        if(e.code().value() == EPERM)
...
jlongArray jni_getresuid(JNIEnv* env, jclass cls)
{
    ...
    try
    {
        util::UserIds uids = util::getUserIds();
...
void jni_setresuid(JNIEnv* env, jclass cls, jlong ruid, jlong euid, jlong suid)
{
    ...
    try
    {
        util::UserIds uids(ruid, euid, suid);
        util::setUserIds(uids);
    }
    catch(std::system_error& e)
    {
        if(e.code().value() == EPERM)
...
jlongArray jni_getresgid(JNIEnv* env, jclass cls)
{
    ...
    try
    {
        util::GroupIds gids = util::getGroupIds();
...
void jni_setresgid(JNIEnv* env, jclass cls, jlong rgid, jlong egid, jlong sgid)
{
    ...
    try
    {
        util::GroupIds gids(rgid, egid, sgid);
        util::setGroupIds(gids);
    }
    catch(std::system_error& e)
    {
        if(e.code().value() == EPERM)
